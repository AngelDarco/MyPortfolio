...
function Projects() {
	...
	const handlerState = (id)=>{
		...
		const arr = [...data];
		arr[i] = { ...arr[i], isClicked: arr[i].isClicked ? !arr[i].isClicked : true};
		setData(arr);
	};
				...
				>
					{
						data?.map((res) => (
							<SwiperSlide className="mainCard" key={res.id}>
								<div className="card">
									<img src={res.img} alt={res.name} onClick={()=>handlerState(res.id)} />
									{
										!res.isClicked && <div className="description">
											<h1>{res.name}</h1>
											<div className="about">
